...
switch($_GET[act]){
	default:
		...
		if (isset($_GET['filter'])) {
			// KALAU VARIABEL FILTER ADA DAN DISET
			$filter = $_GET['filter'];
			$tampil=mysql_query("select * from pelatihan where topik_pelatihan like '$filter%' ");		
			print_r($tampil);
			//print_r($filter);
	   ...
	   <form class='form-inline' action='' method='get' enctype='multipart/form-data'>
		  <div class='form-group'>
		  		 ...
		  		 <input type='hidden' name='module' value='pelatihan' >
			    <input type='text' class='form-control' id='idcari' name='filter' placeholder='cari...' >
...
<table class='tabel'>
  ...
  echo "<tr>
	...
	<td><span><a href='?module=pelatihan&act=edit&id=$dt[id_pelatihan]'>Ubah</a></span><span>
...
echo "  
	...
	case "input":
	echo "<h2 class='head'>Tambah Data Pelatihan</h2>
	...
	echo "<h2 class='head'>Ubah Data Pelatihan</h2>
	...
	<tr>
	<td></td><td></td><td><input type=submit value=Ubah>
	<input type=button value=Batal onclick=self.history.back()>
	</td>
	</tr>
	</table>
